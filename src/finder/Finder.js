...
const ATTACK_DIRECTIONS_STRINGS = ["LEFT", "UP", "RIGHT", "DOWN"];
...
export const Finder = {
  ...
  exploreSimpleSteps: function(surakarta, basePosition, freedoms) {
    if (surakarta.state(basePosition) === NOT_FILLED) {
      throw new Error(
        "Illegal operation: Pebble doesn't exist at given position."
      );
    }
  ...
  exploreAll(surakarta, basePosition, moves) {
    ...
    Finder.exploreSimpleSteps(
      surakarta,
      basePosition,
      moves.steppableDirections
    );
